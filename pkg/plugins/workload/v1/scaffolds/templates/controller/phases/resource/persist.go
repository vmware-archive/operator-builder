...
var _ machinery.Template = &Persist{}

// Persist scaffolds the resource persist phase methods.
type Persist struct {
...
func (f *Persist) SetTemplateDefaults() error {
	f.Path = filepath.Join("controllers", "phases", "resource", "persist.go")

	f.TemplateBody = persistTemplate

	return nil
}

const persistTemplate = `{{ .Boilerplate }}
...
// Persist.Execute executes persisting resources to the Kubernetes database.
func (phase *Persist) Execute(
...
func persistResource(
	...
	if err := r.CreateOrUpdate(resource.GetObject()); err != nil {
		if controllerutils.IsOptimisticLockError(err) {
			return nil
		} else {
